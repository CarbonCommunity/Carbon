...
echo "%TARGET%" | findstr /C:"Unix" >NUL && (
	...
	"%ROOT%\Tools\BuildInfo\Carbon.BuildInfo.exe" -carbon "%ROOT%\Release\.tmp\%TARGET%\carbon\managed\Carbon" -o "%ROOT%\Release\build-unix.info"
	(CALL )
) || (                                                                                                                          
	...
	"%ROOT%\Tools\BuildInfo\Carbon.BuildInfo.exe" -carbon "%ROOT%\Release\.tmp\%TARGET%\carbon\managed\Carbon" -o "%ROOT%\Release\build.info"
	(CALL )
)
