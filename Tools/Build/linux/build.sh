...
if [[ "${TARGET}" == *"Unix"* ]]; then
	...
	cp "${ROOT}/Carbon.Core/Carbon.Native/target/x86_64-unknown-linux-gnu/release/libCarbonNative.so" 	"${ROOT}/Release/.tmp/${TARGET}/RustDedicated_Data/x86_64/libCarbonNative.so"
else
	...
	cp "${ROOT}/Carbon.Core/Carbon.Native/target/x86_64-pc-windows-msvc/release/CarbonNative.dll" 		"${ROOT}/Release/.tmp/${TARGET}/RustDedicated_Data/x86_64/CarbonNative.dll"
fi
