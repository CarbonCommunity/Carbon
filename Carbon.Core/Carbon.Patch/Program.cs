		...
		static void Main(string[] args)
		{
			...
			using (var memoryStream = new MemoryStream())
			{
				using (var archive = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
				{
					...
					archive.CreateEntryFromFile("Tools/Helpers/doorstop_config.ini", "doorstop_config.ini");
					archive.CreateEntryFromFile("Tools/UnityDoorStop/winhttp.dll", "winhttp.dll");
					archive.CreateEntryFromFile("Tools/NStrip/NStrip/bin/Release/net452/NStrip.exe", "carbon/tools/NStrip.exe");
				}
			...
			using (var memoryStream = new MemoryStream())
			{
				using (var archive = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
				{
					...
					archive.CreateEntryFromFile("Tools/Helpers/linux_prepatch.sh", "carbon_prepatch.sh");
