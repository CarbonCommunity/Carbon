		...
		try
		{
			...
			if (!module.Types.Contains(dataType))
			{
				...
				void HandleInitializer()
				{
					...
					var mainConstructor = new MethodDefinition(".ctor",
				...
				{
					var rustSaveMethod =
						type.Methods.FirstOrDefault(x => x.Name.Equals("Save", StringComparison.CurrentCulture)) ??
						new MethodDefinition("Save",
					...
					if (!type.Methods.Contains(rustSaveMethod))
					{
						rustSaveMethod.Parameters.Add(new Mono.Cecil.ParameterDefinition("info",
							Mono.Cecil.ParameterAttributes.None, saveInfoType));
					...
					var instr = rustSaveMethod.Body.Instructions.IndexOf(
						rustSaveMethod.Body.Instructions.FirstOrDefault(x =>
							x.OpCode == OpCodes.Call && x.Operand == save)) + 1;
					il.Body.Instructions.Insert(instr, il.Create(OpCodes.Ldarg_0));
					il.Body.Instructions.Insert(instr + 1, il.Create(OpCodes.Ldarg_0));
					il.Body.Instructions.Insert(instr + 2,
						il.Create(OpCodes.Ldfld, module.ImportReference(carbonDataField)));
					il.Body.Instructions.Insert(instr + 3, il.Create(OpCodes.Ldarg_1));
					var carbonDataLoad = new GenericInstanceMethod(storeModifiers.Methods
						.FirstOrDefault(m => m.Name.Equals("TryUpdateData", StringComparison.CurrentCulture))
						.Resolve());
					carbonDataLoad.GenericArguments.Add(dataType);
					il.Body.Instructions.Insert(instr + 4,
						il.Create(OpCodes.Call, module.ImportReference(carbonDataLoad)));
				}
				...
				void HandleRustLoad()
				{
					...
					il.Body.Instructions.Insert(instr + 3, il.Create(OpCodes.Ldarg_1));
					var carbonDataLoad = new GenericInstanceMethod(storeModifiers.Methods
						.FirstOrDefault(m => m.Name.Equals("TryGetData", StringComparison.CurrentCulture)).Resolve());
					carbonDataLoad.GenericArguments.Add(dataType);
					il.Body.Instructions.Insert(instr + 4,
						il.Create(OpCodes.Call, module.ImportReference(carbonDataLoad)));
				}
