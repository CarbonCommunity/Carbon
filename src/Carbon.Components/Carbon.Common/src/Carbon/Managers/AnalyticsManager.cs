...
/// <summary>
/// Anonymous usage analytics (version, platform, plugin counts). Opt out through config.json.
/// </summary>
public class AnalyticsManager : FacepunchBehaviour
{
	/// <summary>Anonymous, persistent server identifier stored in the server identity folder.</summary>
	...
	{
		return typeof(Community).Assembly.GetCustomAttribute<AssemblyInformationalVersionAttribute>().InformationalVersion;
	});
	...
	{
		return typeof(Community).Assembly.GetCustomAttribute<AssemblyFileVersionAttribute>().Version;
	});
	...
	{
		return typeof(Community).Assembly.GetName().Version.ToString();
	});
	...
	private static readonly Lazy<Identity> _serverInfo = new(() =>
	{
		...
		try
		{
			_location = Path.Combine(Defines.GetRustRootFolder(), "server", ConVar.Server.identity, "carbon.id");
			...
			Services.Analytics.HasNewIdentifier = true;
	...
	public void Awake()
	{
		...
		if (!BootConfig.Singleton.Analytics.Enabled)
		{
