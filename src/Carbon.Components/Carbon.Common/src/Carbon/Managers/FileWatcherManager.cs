...
/// <summary>
/// Shared file system watchers for addon folders (extensions, modules). Events fire on the main thread once enabled.
/// </summary>
public class FileWatcherManager : FacepunchBehaviour, IDisposable
{
	...
	private void DispatchInitialEvents(WatchEntry entry)
	{
		...
		foreach (string file in Directory.EnumerateFiles(entry.Handler.Path, entry.Handler.Filter))
		{
			...
			{
				Logger.Error($"Initial event dispatch failed for '{file}'", ex);
			}
		}
	}
	...
	public void Watch(WatchFolder item)
	{
		...
		{
			Logger.Error("Unable to instantiate a new folder watcher", e);
			throw;
		}
	...
	private void OnWatcherError(object sender, ErrorEventArgs e)
	{
		...
		Logger.Error($"FileSystemWatcher error in '{directory}': {ex?.Message}", ex);
	}
