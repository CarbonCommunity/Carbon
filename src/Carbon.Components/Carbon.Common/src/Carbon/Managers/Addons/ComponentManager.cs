...
/// <summary>
/// Loads Carbon components (ICarbonComponent) from carbon/managed and packages from carbon/managed/packages.
/// </summary>
public class ComponentManager : AddonManager
{
	/*
	 * CARBON COMPONENTS
	 * Carbon.ICarbonComponent
	 *
	...
	/// <summary>Loads every package in carbon/managed/packages. Runs before Carbon.dll so packages can extend it.</summary>
	public void LoadPackages()
	{
		foreach (var file in Directory.GetFiles(Defines.GetPackagesFolder(), "*.dll"))
		{
			Logger.Log($"Loading package '{Path.GetFileName(file)}'");
			Load(Path.GetFileName(file), "ComponentManager.LoadPackages");
		}
	}
						...
						if (types != null)
						{
							foreach (Type type in types)
							{
								try
								{
									...
									arg.Init(file);
									Services.Events.Trigger(CarbonEvent.ComponentLoaded, arg);
									Pool.Free(ref arg);
