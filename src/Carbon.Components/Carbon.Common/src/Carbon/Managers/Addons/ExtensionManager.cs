...
/// <summary>
/// Loads extensions (ICarbonExtension) from carbon/extensions, hot-reloading them on change.
/// </summary>
public class ExtensionManager : AddonManager
{
	...
	internal void Awake()
	{
		Services.FileWatcher.Watch(Watcher = new WatchFolder
		{
			...
			Directory = Defines.GetExtensionsFolder(),
	...
	public override Assembly Load(string file, string requester = null)
	{
		...
		result = _loader.Load(file, requester, _directories, AddonType.Extension)?.Assembly;
		...
		{
			var moduleFile = Path.Combine(Defines.GetExtensionsFolder(), $"{assemblyName}.dll");
			...
			if (types != null)
			{
				...
				{
					if (!typeof(ICarbonExtension).IsAssignableFrom(type))
					{
						continue;
					}
		...
		try
		{
			...
			Services.Events.Trigger(CarbonEvent.ExtensionLoaded, arg);
		...
		catch (Exception e)
		{
			...
			Services.Events.Trigger(CarbonEvent.ExtensionLoadFailed, arg);
		}
	...
	public override void Unload(string file, string requester)
	{
		...
		try
		{
			Services.Events.Trigger(CarbonEvent.ExtensionUnloaded, arg);
		...
		catch (Exception ex)
		{
			...
			Services.Events.Trigger(CarbonEvent.ExtensionUnloadFailed, arg);
		}
