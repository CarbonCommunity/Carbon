...
public readonly struct CUI : IDisposable
{
    ...
    #region Methods
    public CuiElementContainer CreateContainer(string panel, string color = "0 0 0 0", float xMin = 0f, float xMax = 1f, float yMin = 0f, float yMax = 1f, float OxMin = 0f, float OxMax = 0f, float OyMin = 0f, float OyMax = 0f, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, ClientPanels parent = ClientPanels.Overlay, string destroyUi = null, bool activeSelf = true, float rotation = 0, bool? blocksRaycast = null)
    {
    ...
    public CuiElementContainer CreateContainerParent(string panel, string color = "0 0 0 0", float xMin = 0f, float xMax = 1f, float yMin = 0f, float yMax = 1f, float OxMin = 0f, float OxMax = 0f, float OyMin = 0f, float OyMax = 0f, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string parentName = "Overlay", string destroyUi = null, bool activeSelf = true, float rotation = 0, bool? blocksRaycast = null)
	{
		...
		if (!string.IsNullOrEmpty(color) && color != "0 0 0 0")
		{
			...
			image.Color = color;
			image.FadeIn = fadeIn;
			image.BlocksRaycast = blocksRaycast;
	...
	{
		return Manager.Button(container, parent, color, textColor, text, size, material, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, command, align, font, true, fadeIn, fadeOut, needsCursor, needsKeyboard, outlineColor, outlineDistance, outlineUseGraphicAlpha, id, destroyUi, update, activeSelf, rotation, interactable, blocksRaycast);
	}
	...
	{
		return Manager.InputField(container, parent, color, text, size, characterLimit, readOnly, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, command, align, font, false, autoFocus, hudMenuInput, lineType, fadeIn, fadeOut, needsCursor, needsKeyboard, id, destroyUi, update, activeSelf, rotation, interactable, blocksRaycast);
	}
	...
	{
		return Manager.InputField(container, parent, color, text, size, characterLimit, readOnly, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, command, align, font, true, autoFocus, hudMenuInput, lineType, fadeIn, fadeOut, needsCursor, needsKeyboard, id, destroyUi, update, activeSelf, rotation, interactable, blocksRaycast);
	}
	...
	public Pair<string, CuiElement> CreateImage(CuiElementContainer container, string parent, string url, string color, string material = null, float xMin = 0f, float xMax = 1f, float yMin = 0f, float yMax = 1f, float OxMin = 0f, float OxMax = 0f, float OyMin = 0f, float OyMax = 0f, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string outlineColor = null, string outlineDistance = null, bool outlineUseGraphicAlpha = false, string id = null, string destroyUi = null, bool update = false, bool activeSelf = true, float rotation = 0, bool? blocksRaycast = null)
	{
		if (!HasImage(url))
		{
			return Manager.Panel(container, parent, color, material, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax,
				...
				outlineUseGraphicAlpha, id, destroyUi, update, activeSelf, rotation, blocksRaycast);
		}
	...
	public Pair<string, CuiElement> CreateQRCodeImage(CuiElementContainer container, string parent, string text, string brandUrl, string brandColor, string brandBgColor, int pixels, bool transparent, bool quietZones, string color, float xMin = 0f, float xMax = 1f, float yMin = 0f, float yMax = 1f, float OxMin = 0f, float OxMax = 0f, float OyMin = 0f, float OyMax = 0f, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string outlineColor = null, string outlineDistance = null, bool outlineUseGraphicAlpha = false, string id = null, string destroyUi = null, bool update = false, bool activeSelf = true, bool? blocksRaycast = null)
	{
		try
		{
			...
			var qr = CreateImage(container, parent, codeImage, color, null, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin,
				...
				outlineUseGraphicAlpha, id, destroyUi, update, activeSelf, blocksRaycast: blocksRaycast);
			if (!string.IsNullOrEmpty(brandUrl))
			{
				var panel = CreatePanel(container, qr, brandBgColor,
					xMin: 0.4f, xMax: 0.6f, yMin: 0.4f, yMax: 0.6f, blocksRaycast: blocksRaycast);

				CreateImage(container, panel, url: brandUrl, color: brandColor,
					material: null, xMin: 0.15f, 0.85f, yMin: 0.15f, yMax: 0.85f, blocksRaycast: blocksRaycast);
			}
			return qr;
		}
		catch (Exception)
		{
			var qr = CreatePanel(container, parent, Cache.CUI.WhiteColor, color, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin,
				...
				outlineUseGraphicAlpha, id, destroyUi, update, activeSelf, blocksRaycast: blocksRaycast);
			CreateText(container, qr, "0 0 0 0.5", "GDI+ is not installed!", 10, blocksRaycast: blocksRaycast);
			return qr;
		}
	}
	...
	public CuiTooltipComponent CreateTooltip(CuiElement element, string text, CommunityEntity.TooltipType? tooltipType = null, string offset = null, bool useCentre = false, Tooltip.DelayType? delay = null, TooltipContainer.PositionMode? position = null, bool update = false)
	{
	...
	public class Handler
	{
		...
		private List<ICuiComponent> _canvasGroups = new();
		private List<ICuiComponent> _masks = new();
		private List<ICuiComponent> _tooltips = new();
		...
		private CuiCanvasGroupComponent _defaultCanvasGroup = new();
		private CuiMaskComponent _defaultMask = new();
		private CuiTooltipComponent _defaultTooltip = new();
		...
		public void SendToPool<T>(T element) where T : ICuiComponent
		{
			...
			switch (element)
			{
				...
				case CuiCanvasGroupComponent: _canvasGroups.Add(element); break;
				case CuiMaskComponent: _masks.Add(element); break;
				case CuiTooltipComponent: _tooltips.Add(element); break;
			}
		}
		...
		public CuiImageComponent TakeFromPoolImage()
		{
			...
			else
			{
				...
				element.SkinId = _defaultImage.SkinId;
				element.PpuMultiplier = _defaultImage.PpuMultiplier;
				element.PlaceholderParentId = _defaultImage.PlaceholderParentId;
				element.BlocksRaycast = _defaultImage.BlocksRaycast;
		...
		public CuiRawImageComponent TakeFromPoolRawImage()
		{
			...
			else
			{
				...
				element.BlocksRaycast = _defaultRawImage.BlocksRaycast;
		...
		public CuiTextComponent TakeFromPoolText()
		{
			...
			else
			{
				...
				element.BlocksRaycast = _defaultText.BlocksRaycast;
		...
		public CuiButtonComponent TakeFromPoolButton()
		{
			...
			else
			{
				...
				element.Interactable = _defaultButton.Interactable;
				element.PlaceholderParentId = _defaultButton.PlaceholderParentId;
				element.BlocksRaycast = _defaultButton.BlocksRaycast;
		...
		public CuiInputFieldComponent TakeFromPoolInputField()
		{
			...
			else
			{
				...
				element.Interactable = _defaultInputField.Interactable;
				element.PlaceholderParentId = _defaultInputField.PlaceholderParentId;
				element.BlocksRaycast = _defaultInputField.BlocksRaycast;
		...
		{
			var element = (CuiCanvasGroupComponent)null;

			if (_canvasGroups.Count == 0)
			{
				element = new CuiCanvasGroupComponent();
			}
			else
			{
				element = (_canvasGroups[0] as CuiCanvasGroupComponent)!;
				element.Alpha = _defaultCanvasGroup.Alpha;
				element.BlocksRaycasts = _defaultCanvasGroup.BlocksRaycasts;
				element.Interactable = _defaultCanvasGroup.Interactable;
				element.Enabled = _defaultCanvasGroup.Enabled;
		...
		{
			var element = (CuiMaskComponent)null;

			if (_masks.Count == 0)
			{
				element = new CuiMaskComponent();
			}
			else
			{
				element = (_masks[0] as CuiMaskComponent)!;
				element.ShowMaskGraphic = _defaultMask.ShowMaskGraphic;
				element.Enabled = _defaultMask.Enabled;
				_masks.RemoveAt(0);
			}
		...
		{
			var element = (CuiTooltipComponent)null;

			if (_tooltips.Count == 0)
			{
				element = new CuiTooltipComponent();
			}
			else
			{
				element = (_tooltips[0] as CuiTooltipComponent)!;
				element.TooltipType = _defaultTooltip.TooltipType;
				element.Offset = _defaultTooltip.Offset;
				element.UseCentre = _defaultTooltip.UseCentre;
				element.Text = _defaultTooltip.Text;
				element.Delay = _defaultTooltip.Delay;
				element.Position = _defaultTooltip.Position;
				element.Enabled = _defaultTooltip.Enabled;
...
public static class CUIStatics
{
	...
	public static Pair<string, CuiElement> UpdatePanel(this CUI cui, string id, string color, string material = null, float xMin = 0f, float xMax = 1f, float yMin = 0f, float yMax = 1f, float OxMin = 0f, float OxMax = 0f, float OyMin = 0f, float OyMax = 0f, bool blur = false, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string outlineColor = null, string outlineDistance = null, bool outlineUseGraphicAlpha = false, string destroyUi = null, bool activeSelf = true, float rotation = 0, bool? blocksRaycast = null)
	{
		return cui.CreatePanel(null, null, color, material, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, blur, fadeIn, fadeOut, needsCursor, needsKeyboard, outlineColor, outlineDistance, outlineUseGraphicAlpha, id, destroyUi, true, activeSelf, rotation, blocksRaycast);
	}
	public static Pair<string, CuiElement> UpdateText(this CUI cui, string id, string color, string text, int size, float xMin = 0f, float xMax = 1f, float yMin = 0f, float yMax = 1f, float OxMin = 0f, float OxMax = 0f, float OyMin = 0f, float OyMax = 0f, TextAnchor align = TextAnchor.MiddleCenter, Handler.FontTypes font = Handler.FontTypes.RobotoCondensedRegular, VerticalWrapMode verticalOverflow = VerticalWrapMode.Overflow, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string outlineColor = null, string outlineDistance = null, bool outlineUseGraphicAlpha = false, string destroyUi = null, bool activeSelf = true, float rotation = 0, bool? blocksRaycast = null)
	{
		return cui.CreateText(null, null, color, text, size, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, align, font, verticalOverflow, fadeIn, fadeOut, needsCursor, needsKeyboard, outlineColor, outlineDistance, outlineUseGraphicAlpha, id, destroyUi, true, activeSelf, rotation, blocksRaycast);
	}
	public static Pair<string, CuiElement, CuiElement> UpdateButton(this CUI cui, string id, string color, string textColor, string text, int size, string material = null, float xMin = 0f, float xMax = 1f, float yMin = 0f, float yMax = 1f, float OxMin = 0f, float OxMax = 0f, float OyMin = 0f, float OyMax = 0f, string command = null, TextAnchor align = TextAnchor.MiddleCenter, Handler.FontTypes font = Handler.FontTypes.RobotoCondensedRegular, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string outlineColor = null, string outlineDistance = null, bool outlineUseGraphicAlpha = false, string destroyUi = null, bool activeSelf = true, float rotation = 0, bool? interactable = null, bool? blocksRaycast = null)
	{
		return cui.CreateButton(null, null, color, textColor, text, size, material, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, command, align, font, fadeIn, fadeOut, needsCursor, needsKeyboard, outlineColor, outlineDistance, outlineUseGraphicAlpha, id, destroyUi, true, activeSelf, rotation, interactable, blocksRaycast);
	}
	...
	{
		return cui.CreateProtectedButton(null, null, color, textColor, text, size, material, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, command, align, font, fadeIn, fadeOut, needsCursor, needsKeyboard, outlineColor, outlineDistance, outlineUseGraphicAlpha, id, destroyUi, true, activeSelf, rotation, interactable, blocksRaycast);
	}
	...
	{
		return cui.CreateInputField(null, null, color, text, size, characterLimit, readOnly, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, command, align, font, autoFocus, hudMenuInput, lineType, fadeIn, fadeOut, needsCursor, needsKeyboard, id, destroyUi, true, activeSelf, rotation, interactable, blocksRaycast);
	}
	public static Pair<string, CuiElement> UpdateProtectedInputField(this CUI cui, string id, string color, string text, int size, int characterLimit, bool readOnly, float xMin = 0f, float xMax = 1f, float yMin = 0f, float yMax = 1f, float OxMin = 0f, float OxMax = 0f, float OyMin = 0f, float OyMax = 0f, string command = null, TextAnchor align = TextAnchor.MiddleCenter, Handler.FontTypes font = Handler.FontTypes.RobotoCondensedRegular, bool autoFocus = false, bool hudMenuInput = false, InputField.LineType lineType = UnityEngine.UI.InputField.LineType.SingleLine, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string destroyUi = null, bool activeSelf = true, float rotation = 0, bool? interactable = null, bool? blocksRaycast = null)
	{
		return cui.CreateProtectedInputField(null, null, color, text, size, characterLimit, readOnly, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, command, align, font, autoFocus, hudMenuInput, lineType, fadeIn, fadeOut, needsCursor, needsKeyboard, id, destroyUi, true, activeSelf, rotation, interactable, blocksRaycast);
	}
	...
	{
		return cui.CreateImage(null, null, png, color, material, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, fadeIn, fadeOut, needsCursor, needsKeyboard, outlineColor, outlineDistance, outlineUseGraphicAlpha, id, destroyUi, true, activeSelf, rotation, blocksRaycast);
	}
	...
	{
		return cui.CreateImage(null, null, url, color, material, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, fadeIn, fadeOut, needsCursor, needsKeyboard, outlineColor, outlineDistance, outlineUseGraphicAlpha, id, destroyUi, true, activeSelf, rotation, blocksRaycast);
	}
	...
	{
		return cui.CreateSimpleImage(null, null, png, sprite, color, material, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, fadeIn, fadeOut, needsCursor, needsKeyboard, outlineColor, outlineDistance, outlineUseGraphicAlpha, id, destroyUi, true, activeSelf, rotation, slice, ppuMultiplier, blocksRaycast);
	}
	...
	{
		return cui.CreateSprite(null, null, sprite, color, material, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, fadeIn, fadeOut, needsCursor, needsKeyboard, outlineColor, outlineDistance, outlineUseGraphicAlpha, id, destroyUi, true, activeSelf, rotation, blocksRaycast);
	}
	public static Pair<string, CuiElement> UpdateItemImage(this CUI cui, string id, int itemID, ulong skinID, string color, string material = null, float xMin = 0f, float xMax = 1f, float yMin = 0f, float yMax = 1f, float OxMin = 0f, float OxMax = 0f, float OyMin = 0f, float OyMax = 0f, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string outlineColor = null, string outlineDistance = null, bool outlineUseGraphicAlpha = false, string destroyUi = null, bool activeSelf = true, float rotation = 0, float? ppuMultiplier = null, bool? blocksRaycast = null)
	{
		return cui.CreateItemImage(null, null, itemID, skinID, color, material, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, fadeIn, fadeOut, needsCursor, needsKeyboard, outlineColor, outlineDistance, outlineUseGraphicAlpha, id, destroyUi, true, activeSelf, rotation, ppuMultiplier, blocksRaycast);
	}
	...
	{
		return cui.CreateQRCodeImage(null, null, text, brandUrl, brandColor, brandBgColor, pixels, transparent, quietZones, color, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, fadeIn, fadeOut, needsCursor, needsKeyboard, outlineColor, outlineDistance, outlineUseGraphicAlpha, id, destroyUi, true, activeSelf, blocksRaycast);
	}
	...
	{
		return cui.CreateClientImage(null, null, url, color, material, xMin, xMax, yMin, yMax, OxMin, OxMax, OyMin, OyMax, fadeIn, fadeOut, needsCursor, needsKeyboard, outlineColor, outlineDistance, outlineUseGraphicAlpha, id, destroyUi, true, activeSelf, rotation, blocksRaycast);
	}
	...
	{
		return cui.CreateCanvasGroup(element, alpha, blocksRaycasts, interactable, true);
	}
	public static CuiMaskComponent UpdateMask(this CUI cui, CuiElement element, bool? showMaskGraphic = null)
	{
		return cui.CreateMask(element, showMaskGraphic, true);
	}
	...
	{
		return cui.CreateTooltip(element, text, tooltipType, offset, useCentre, delay, position, true);
	}
	...
	public static Pair<string, CuiElement> Text(this Handler cui, CuiElementContainer container, string parent, string color, string text, int size, float xMin, float xMax, float yMin, float yMax, float OxMin, float OxMax, float OyMin, float OyMax, TextAnchor align, Handler.FontTypes font, VerticalWrapMode verticalOverflow, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string outlineColor = null, string outlineDistance = null, bool outlineUseGraphicAlpha = false, string id = null, string destroyUi = null, bool update = false, bool activeSelf = true, float rotation = 0, bool? blocksRaycast = null)
	{
		...
		label.BlocksRaycast = blocksRaycast;
	...
	public static Pair<string, CuiElement, CuiElement> Button(this Handler cui, CuiElementContainer container, string parent, string color, string textColor, string text, int size, string material, float xMin, float xMax, float yMin, float yMax, float OxMin, float OxMax, float OyMin, float OyMax, string command, TextAnchor align, Handler.FontTypes font, bool @protected, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string outlineColor = null, string outlineDistance = null, bool outlineUseGraphicAlpha = false, string id = null, string destroyUi = null, bool update = false, bool activeSelf = true, float rotation = 0, bool? interactable = null, bool? blocksRaycast = null)
	{
		...
		button.BlocksRaycast = blocksRaycast;
		...
		if (!string.IsNullOrEmpty(text))
		{
			...
			ptext.BlocksRaycast = blocksRaycast;
	...
	public static Pair<string, CuiElement> InputField(this Handler cui, CuiElementContainer container, string parent, string color, string text, int size, int characterLimit, bool readOnly, float xMin, float xMax, float yMin, float yMax, float OxMin, float OxMax, float OyMin, float OyMax, string command, TextAnchor align, Handler.FontTypes font, bool @protected, bool autoFocus = false, bool hudMenuInput = false, InputField.LineType lineType = UnityEngine.UI.InputField.LineType.SingleLine, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string id = null, string destroyUi = null, bool update = false, bool activeSelf = true, float rotation = 0, bool? interactable = null, bool? blocksRaycast = null)
	{
		...
		inputField.BlocksRaycast = blocksRaycast;
	...
	public static Pair<string, CuiElement> Image(this Handler cui, CuiElementContainer container, string parent, string png, string url, string steamId, string color, string material, float xMin, float xMax, float yMin, float yMax, float OxMin, float OxMax, float OyMin, float OyMax, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string outlineColor = null, string outlineDistance = null, bool outlineUseGraphicAlpha = false, string id = null, string destroyUi = null, bool update = false, bool activeSelf = true, float rotation = 0, bool? blocksRaycast = null)
	{
		...
		rawImage.BlocksRaycast = blocksRaycast;
	...
	public static Pair<string, CuiElement> SimpleImage(this Handler cui, CuiElementContainer container, string parent, string png, string sprite, string color, string material, float xMin, float xMax, float yMin, float yMax, float OxMin, float OxMax, float OyMin, float OyMax, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string outlineColor = null, string outlineDistance = null, bool outlineUseGraphicAlpha = false, string id = null, string destroyUi = null, bool update = false, bool activeSelf = true, float rotation = 0, string slice = null, float? ppuMultiplier = null, bool? blocksRaycast = null)
	{
		...
		simpleImage.PpuMultiplier = ppuMultiplier;
		simpleImage.BlocksRaycast = blocksRaycast;
	...
	public static Pair<string, CuiElement> ItemImage(this Handler cui, CuiElementContainer container, string parent, int itemID, ulong skinID, string color, string material, float xMin, float xMax, float yMin, float yMax, float OxMin, float OxMax, float OyMin, float OyMax, float fadeIn = 0f, float fadeOut = 0f, bool needsCursor = false, bool needsKeyboard = false, string outlineColor = null, string outlineDistance = null, bool outlineUseGraphicAlpha = false, string id = null, string destroyUi = null, bool update = false, bool activeSelf = true, float rotation = 0, float? ppuMultiplier = null, bool? blocksRaycast = null)
	{
		...
		rawImage.FadeIn = fadeIn;
		rawImage.Color = ProcessColor(color);
		rawImage.PpuMultiplier = ppuMultiplier;
		rawImage.BlocksRaycast = blocksRaycast;
		if (material != null) rawImage.Material = material;
		element.Components.Add(rawImage);
	...
	public static CuiCanvasGroupComponent CanvasGroup(this Handler cui, CuiElement element, float? alpha, bool? blocksRaycasts, bool? interactable, bool update = false)
	{
	...
	public static CuiMaskComponent Mask(this Handler cui, CuiElement element, bool? showMaskGraphic, bool update = false)
	{
	...
	public static CuiTooltipComponent Tooltip(this Handler cui, CuiElement element, string text, CommunityEntity.TooltipType? tooltipType, string offset, bool useCentre, Tooltip.DelayType? delay, TooltipContainer.PositionMode? position, bool update = false)
	{
