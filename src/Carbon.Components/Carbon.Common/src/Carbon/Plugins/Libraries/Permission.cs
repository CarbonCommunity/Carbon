...
/// <summary>
/// Users, groups and permissions. Protobuf-backed by default, see PermissionSql and PermissionStoreless for the other backends.
/// </summary>
public class Permission : Library
{
	...
	/// <summary>Fired after a connected player's user data got refreshed (name, language, default groups).</summary>
	public event Action<BasePlayer, UserData> OnUserRefreshed;

	private const int ParentGroupDepthLimit = 32;

	/// <summary>Protobuf files in carbon/data holding users and groups.</summary>
	public virtual string UsersFile => "carbon.users";
	public virtual string GroupsFile => "carbon.groups";

	// Data files written by Carbon 2.x, imported once so upgrading servers keep their permissions
	private const string LegacyUsersFile = "oxide.users";
	private const string LegacyGroupsFile = "oxide.groups";

	private void ImportLegacyFiles()
	{
		if (!ProtoStorage.Exists(UsersFile) && ProtoStorage.Exists(LegacyUsersFile))
		{
			File.Copy(ProtoStorage.GetFileDataPath($"{LegacyUsersFile}.data"), ProtoStorage.GetFileDataPath($"{UsersFile}.data"));
			Logger.Warn($"Imported legacy permission users from '{LegacyUsersFile}'");
		}
		if (!ProtoStorage.Exists(GroupsFile) && ProtoStorage.Exists(LegacyGroupsFile))
		{
			File.Copy(ProtoStorage.GetFileDataPath($"{LegacyGroupsFile}.data"), ProtoStorage.GetFileDataPath($"{GroupsFile}.data"));
			Logger.Warn($"Imported legacy permission groups from '{LegacyGroupsFile}'");
		}
	}
	...
	public virtual void LoadFromDatafile()
	{
		...
		ProtoStorage.DatafileToProto<Dictionary<string, UserData>>(UsersFile, true);
		ProtoStorage.DatafileToProto<Dictionary<string, GroupData>>(GroupsFile, true);

		var needsUserSave = false;
		var needsGroupSave = false;

		userdata = (ProtoStorage.Load<Dictionary<string, UserData>>(UsersFile) ?? new Dictionary<string, UserData>(StringComparer.OrdinalIgnoreCase));
		{
		...
		groupdata = (ProtoStorage.Load<Dictionary<string, GroupData>>(GroupsFile) ?? new Dictionary<string, GroupData>());
		{
	...
	public virtual void Export(string prefix = "auth")
	{
		...
		Community.Runtime.DataFileSystem.WriteObject(prefix + ".groups", groupdata, false);
		Community.Runtime.DataFileSystem.WriteObject(prefix + ".users", userdata, false);
	}
	...
	{
		ProtoStorage.Save(userdata, UsersFile);
	}
	public virtual void SaveGroups()
	{
		ProtoStorage.Save(groupdata, GroupsFile);
	}
	...
	public virtual void MigrateGroup(string oldGroup, string newGroup)
	{
		...
		{
			var fileDataPath = ProtoStorage.GetFileDataPath($"{GroupsFile}.data");
	...
	public virtual void RefreshUser(BasePlayer player)
	{
		...
		OnUserRefreshed?.Invoke(player, user);
	}
	public virtual void UpdateNickname(string id, string nickname)
	{
		...
		userData.LastSeenNickname = nickname.Replace("{", "{{").Replace("}", "}}");
