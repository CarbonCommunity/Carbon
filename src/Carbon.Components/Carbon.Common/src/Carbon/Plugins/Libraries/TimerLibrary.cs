...
/// <summary>
/// Timer engine owning every timer of a plugin. Scheduling lives in TimerLibrary.Scheduler.
/// </summary>
public partial class TimerLibrary : Library
{
	public Plugin Plugin { get; }
	internal readonly HashSet<TimerInstance> _timers = [];

	public TimerLibrary() { }
	public TimerLibrary(Plugin plugin)
	{
		Plugin = plugin;
	}
	...
	public class TimerInstance : IDisposable
	{
		...
		// Alias kept for plugins that read the owner as 'Owner'
		public Plugin Owner => Plugin;

		internal TimerLibrary OwnerTimers { get; set; }
		...
		public void Reset(float delay = -1f, int repetitions = 1)
		{
			...
			lock (SchedulerLock)
			{
				...
				TimerLibrary.ScheduleIn(this, Repeating ? TimerLibrary.NormalizeRepeatDelay(delay) : delay);
			}
		}
		public bool Destroy()
		{
