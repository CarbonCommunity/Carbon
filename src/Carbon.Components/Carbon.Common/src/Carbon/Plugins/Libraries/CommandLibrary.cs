...
/// <summary>
/// Registers chat and console commands for plugins (the "cmd" library).
/// </summary>
public class CommandLibrary : Library
{
	/// <summary>
	/// Command methods whose first parameter is one of these types get the caller converted through the factory
	/// instead of a BasePlayer/Arg. Lets compatibility layers bind their own player abstractions.
	/// </summary>
	public static Dictionary<Type, Func<BasePlayer, object>> CallerFactories { get; } = new();

	private static bool IsCallerParameter(ParameterInfo parameter) => CallerFactories.ContainsKey(parameter.ParameterType);

	private static MethodInfo FindCommandMethod(BaseHookable plugin, string method)
	{
		var methodInfos = plugin.GetType().GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
		var callerMethod = methodInfos.FirstOrDefault(x => x.Name == method && (!x.GetParameters().Any() || x.GetParameters().Any(IsCallerParameter)));
		var consoleMethod = methodInfos.FirstOrDefault(x => x.Name == method && (!x.GetParameters().Any() || x.GetParameters().Any(y => !IsCallerParameter(y))));
		return callerMethod ?? consoleMethod;
	}

	private static object CreateCaller(ParameterInfo[] parameters, BasePlayer player)
	{
		var parameter = parameters.First(IsCallerParameter);
		return CallerFactories[parameter.ParameterType](player);
	}
	...
	private Func<Carbon.Commands.Command, Carbon.Commands.Command.Args, bool> OnPlayerExecute(bool isChat)
	{
		return (cmd, args) =>
		{
	...
	{
		var cmd = new Carbon.Commands.Command.Chat
		{
			Name = command,
			Reference = plugin,
			Callback = arg =>
			...
			Help = help,
			Token = reference,
			Auth = new Carbon.Commands.Command.Authentication
			{
				AuthLevel = authLevel,
	...
	{
		var methodInfo = FindCommandMethod(plugin, method);
	...
	public void AddChatCommand(string command, BaseHookable plugin, MethodInfo methodInfo, string help = null, object reference = null, string[] permissions = null, string[] groups = null, int authLevel = -1, int cooldown = 0, bool isHidden = false, bool @protected = false, bool silent = false, bool doCooldownPenalty = false)
	{
		AddChatCommand(command, plugin, (player, cmd, args) =>
		{
			...
			try
			{
				...
				var callerMethod = parameters.Length > 0 && parameters.Any(IsCallerParameter);

				if (parameters.Length > 0)
				{
					if (callerMethod)
					{
						arguments.Add(CreateCaller(parameters, player));
					}
					else
					{
	...
	public void AddConsoleCommand(string command, BaseHookable plugin, Action<BasePlayer, string, string[]> callback, string help = null, object reference = null, string[] permissions = null, string[] groups = null, int authLevel = -1, int cooldown = 0, bool isHidden = false, bool @protected = false, bool silent = false, bool doCooldownPenalty = false)
	{
		// Client console
		{
			var cmd = new Carbon.Commands.Command.ClientConsole
			{
				Name = command,
				Reference = plugin,
				Callback = arg =>
		...
		// RCon console
		{
			var cmd = new Carbon.Commands.Command.RCon
			{
				Name = command,
				Reference = plugin,
				Callback = arg =>
	...
	/// <summary>Registers the same command for chat and console.</summary>
	public void AddCommand(string command, BaseHookable plugin, string method, string help = null, object reference = null, string[] permissions = null, string[] groups = null, int authLevel = -1, int cooldown = 0, bool isHidden = false, bool @protected = false, bool silent = true, bool doCooldownPenalty = false)
	{
	...
	public void AddCommand(string command, BaseHookable plugin, Action<BasePlayer, string, string[]> callback, string help = null, object reference = null, string[] permissions = null, string[] groups = null, int authLevel = -1, int cooldown = 0, bool isHidden = false, bool @protected = false, bool silent = true, bool doCooldownPenalty = false)
	{
