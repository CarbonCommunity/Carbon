...
/// <summary>
/// Per-plugin localization: phrases per language stored in carbon/lang/[lang]/[plugin].json.
/// </summary>
public class Lang : Library
{
	...
	{
		if (!string.IsNullOrEmpty(userId) && Community.Runtime.Permission.UserExists(userId, out var data))
		{
	...
	public string[] GetLanguages(Plugin plugin = null)
	{
		...
		foreach (string text in Directory.GetDirectories(Defines.GetLangFolder()))
		{
			...
			{
				list.Add(text[(Defines.GetLangFolder().Length + 1)..]);
			}
		}
	...
	public void SetLanguage(string lang, string userId)
	{
		...
		var data = Community.Runtime.Permission.GetUserData(userId, true);
