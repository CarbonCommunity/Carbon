...
/// <summary>
/// Free-form JSON file: key/value access plus typed ReadObject/WriteObject. Backs plugin configs and data files.
/// </summary>
public class DynamicConfigFile : ConfigFile, IEnumerable<KeyValuePair<string, object>>, IEnumerable
{
	...
	{
		filename = CheckPath(filename ?? Filename);
		string directoryName = PathEx.GetDirectoryName(filename);
	...
	public void WriteObject<T>(T config, bool sync = false, string filename = null)
	{
		...
		filename = CheckPath(filename ?? Filename);
		var directoryName = PathEx.GetDirectoryName(filename);
		if (directoryName != null && !Directory.Exists(directoryName))
		{
			Directory.CreateDirectory(directoryName);
		}
