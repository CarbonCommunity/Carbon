...
public class OxideTypeRef : BaseOxidePatch
{
    public static List<string> PluginToBaseHookable = new()
	{
        ...
        "System.Void Carbon.Plugins.Lang::RegisterMessages(System.Collections.Generic.Dictionary`2<System.String, System.String>, Oxide.Core.Plugins.Plugin, System.String)",
        //"System.String Carbon.Plugins.Lang::GetMessage(System.String, Oxide.Core.Plugins.Plugin, System.String)",
        "System.Void Carbon.Plugins.CommandLibrary::RemoveConsoleCommand(System.String, Oxide.Core.Plugins.Plugin)",
        "System.Collections.Generic.Dictionary`2<System.String, System.String> Carbon.Plugins.Lang::GetMessages(System.String, Oxide.Core.Plugins.Plugin)"
    };
    ...
    public override void Apply(ModuleDefinition assembly, ReferenceImporter importer, ref BaseConverter.Context context)
    {
        foreach (MemberReference memberReference in assembly.GetImportedMemberReferences())
        {
            ...
            {
	            if (!OxideHelpers.IsOxideASM(aref))
	            {
		            continue;
	            }
	            ...
	            if (PluginToBaseHookable.Contains(memberReference.FullName))
	            {
		            for (int index = 0; index < methodSignature.ParameterTypes.Count; index++)
		            {
			            ...
			            if (typeSig.FullName == "Oxide.Core.Plugins.Plugin" && OxideHelpers.IsOxideASM(typeSig.DefinitionAssembly()))
			            {
    ...
    public static void ProcessTypeRef(TypeReference type, ReferenceImporter importer)
    {
        ...
        if (type.Scope is not AssemblyReference aref || !OxideHelpers.IsOxideASM(aref))
        {
	        return;
        }
        ...
        {
	        type.Scope = OxideHelpers.GetNewtonsoftScope(type, importer);
	        return;
        }
        ...
        // Natively provided types move to Carbon's own namespaces, everything else lives in this package
        var mapped = OxideTypeMap.Map(type.FullName);
        if (mapped != null)
        {
	        var split = mapped.LastIndexOf('.');
	        type.Namespace = mapped.Substring(0, split);
	        type.Name = mapped.Substring(split + 1);
	        type.Scope = (OxideTypeMap.SdkTypes.Contains(mapped) ? CompatManager.SDK : CompatManager.Common).ImportWith(importer);
	        return;
        }

        oxide:
        type.Scope = OxideHelpers.OxideAssembly.ImportWith(importer);
        return;
