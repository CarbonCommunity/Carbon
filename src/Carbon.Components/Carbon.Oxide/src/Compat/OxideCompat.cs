...
public static partial class OxideCompat
{
    ...
    public static void RegisterPluginLoader(Oxide.Core.Extensions.ExtensionManager self, PluginLoader loader, Extension oxideExt)
    {
        ...
        foreach (Type type in loader.CorePlugins)
        {
            ...
            try
            {
                ModLoader.InitializePlugin(type, out Plugin plugin, package, precompiled: true, preInit: oxideExt == null ? null :
                    rustPlugin =>
                    {
    ...
    public static string GetMessage1(global::Carbon.Plugins.Lang lang, string key, Plugin plugin, string player)
    {
	    return lang.GetMessage(key, plugin, player);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void AddConsoleCommand1(global::Carbon.Plugins.CommandLibrary cmd, string name, Plugin plugin, Func<ConsoleSystem.Arg, bool> callback)
    {
        cmd.AddConsoleCommand(name, plugin, callback);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void AddChatCommand1(global::Carbon.Plugins.CommandLibrary cmd, string name, Plugin plugin, Action<BasePlayer, string, string[]> callback)
    {
